...
interface VideoEditorProps {
  onThumbnailChange?: (data: { thumbnailUrl: string; cleanBackgroundUrl?: string; playlistPosition: { x: number; y: number } }) => void;
}
...
export const VideoEditor = forwardRef<VideoEditorRef, VideoEditorProps>(({ onThumbnailChange }, ref) => {
  ...
  const renderThumbnail = (hidePlaylist = false): string | null => {
    ...
    // Only set these if not already correct to avoid flicker/reset
    // But for this export function assume we redraw
    ...
    if (showTitle && title) {
      ...
      // Only draw selection if we are NOT generating the clean background
      if (selectedComponent === 'title' && !hidePlaylist) {
